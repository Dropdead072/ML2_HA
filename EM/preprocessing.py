...
# https://lxml.de/apidoc/lxml.objectify.html
...
def split_function(string):
    if len(string) != 0:
        ...
        return (int(split_arr[0]), int(split_arr[1]))
    else:
        return ()
...
def extract_sentences(filename: str) -> Tuple[List[SentencePair], List[LabeledAlignment]]:
    ...
    for sentence in root.getchildren():
        sentence_objects.append( SentencePair( str(sentence.find('english')).split(' '), str(sentence.find('czech')).split(' ')) )
        alignment_objects.append( LabeledAlignment( list(map(split_function, str(sentence.find('sure')).split(' '))),
                                                   list(map(split_function, str(sentence.find('possible')).split(' ')))) )

    return ( sentence_objects, alignment_objects )
